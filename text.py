    ...
    def change_color(self, color):
        ...
        for c in self.text:
            ...
            for i, j in itertools.product(range(7), range(7)):
                if (letter.get_at([i, j]) != pygame.Color(0, 0, 0, 0)):
                    letter.set_at([i, j], self._text_color)

    def draw(self, surf : pygame.SurfaceType, pos : list[float], scale = 1.0, shaking = True, shadowing = True):
        x = 0
        y = 0
        dx = 0
        dy = 0
